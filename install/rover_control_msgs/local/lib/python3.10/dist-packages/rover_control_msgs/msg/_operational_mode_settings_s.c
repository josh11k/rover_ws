...
bool rover_control_msgs__msg__operational_mode_settings__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // make_global_pointcloud
    PyObject * field = PyObject_GetAttrString(_pymsg, "make_global_pointcloud");
    if (!field) {
      return false;
    }
    assert(PyUnicode_Check(field));
    ...
    rosidl_runtime_c__String__assign(&ros_message->make_global_pointcloud, PyBytes_AS_STRING(encoded_field));
    Py_DECREF(encoded_field);
    Py_DECREF(field);
  }
...
PyObject * rover_control_msgs__msg__operational_mode_settings__convert_to_py(void * raw_ros_message)
{
  ...
  {  // make_global_pointcloud
    PyObject * field = NULL;
    field = PyUnicode_DecodeUTF8(
      ros_message->make_global_pointcloud.data,
      strlen(ros_message->make_global_pointcloud.data),
      "replace");
    if (!field) {
      return NULL;
    }
    {
      int rc = PyObject_SetAttrString(_pymessage, "make_global_pointcloud", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
